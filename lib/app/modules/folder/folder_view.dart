...
class FolderView extends GetView<FolderController> {
                ...
                return Column(
                  children: [
                    ...
                    Obx(() => controller.isICloudExpanded.value 
                      ? _buildFolderGroup(controller.iCloudFolders, isICloud: true) 
                    ...
                    Obx(() => controller.isOnMyiPhoneExpanded.value 
                      ? _buildFolderGroup(controller.onMyiPhoneFolders, isICloud: false, includeRecentlyDeleted: true) 
                      : const SizedBox.shrink()),
  ...
  Widget _buildFolderGroup(List<FolderModel> folders, {required bool isICloud, bool includeRecentlyDeleted = false}) {
    return Padding(
        ...
        child: Column(
          children: [
            for (int i = 0; i < folders.length; i++) ...[
              ...
              if (i < folders.length - 1 || includeRecentlyDeleted)
                const Divider(indent: 56, height: 1),
            ],
            if (includeRecentlyDeleted && controller.deletedCount.value > 0)
              _buildRecentlyDeletedTile(),
          ],
        ),
      ),
    );
    ...
    return Obx(() {
      ...
      return Opacity(
        ...
        child: ListTile(
          ...
          onTap: (isEditing && isSystem) ? null : () => Get.toNamed(Routes.NOTE_LIST, arguments: folder)
          ...
          title: Text(folder.name, 
            style: const TextStyle(fontSize: 17, color: AppTheme.textPrimary, fontWeight: FontWeight.w400)),
